...
WARTERAUM_NAME = "Warteraum"  # Exakter Name des Voice-Channels
TEAM_NOTIFICATION_KANAL = "team-notifikation" # Kanal für die Team-Nachricht

# Direkter Link zu einer Musik-Datei (Standard: Fahrstuhlmusik-MP3)
WARTERAUM_MUSIK_URL = "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3"
...
def play_warteraum_music(vc):
    ...
    def after_playing(error):
        ...
        # Wenn der Bot noch im Sprachkanal ist, Musik von vorn starten
        if vc.is_connected():
...
async def handle_warteraum(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState, bot: discord.Client):
    """Prüft, ob jemand den Warteraum betritt oder verlässt."""
    if member.bot:
        return

    # 1. USER BETRITT DEN WARTERAUM
    if after.channel and after.channel.name == WARTERAUM_NAME:
        ...
        if not vc:
            try:
                ...
                print(f"Konnte dem Warteraum nicht beitreten: {e}")
                return
        ...
        if team_kanal:
            embed = discord.Embed(
                title="🔔 JEMAND WARTET IM WARTERAUM!",
    ...
    if before.channel and before.channel.name == WARTERAUM_NAME:
        ...
        # Wenn kein normaler User mehr im Raum ist -> Bot verlässt den Channel
        if len(echte_user) == 0:
            vc = discord.utils.get(bot.voice_clients, guild=guild)
            if vc:
                await vc.disconnect()
