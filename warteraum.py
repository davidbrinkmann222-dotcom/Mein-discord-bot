...
async def handle_warteraum(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState, bot: discord.Client):
    ...
    if after.channel and after.channel.name == WARTERAUM_NAME:
        ...
        print(f"🔍 DEBUG: {member.name} hat den Warteraum '{voice_channel.name}' betreten.")
        ...
        if not vc:
            try:
                ...
                print("✅ DEBUG: Bot hat sich erfolgreich mit dem Warteraum-Voice verbunden!")
            except Exception as e:
    ...
    if before.channel and before.channel.name == WARTERAUM_NAME:
        ...
        guild = member.guild
        print(f"🔍 DEBUG: {member.name} hat den Warteraum '{voice_channel.name}' verlassen.")
